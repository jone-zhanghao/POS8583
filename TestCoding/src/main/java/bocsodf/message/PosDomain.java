...
public class PosDomain {
    ...
    public PosDomain(){
        posMap.put("FIELD002",new DomainInfo(PosTypeEnum.INTEGER,"VAR2"));//primary_acct_num 主账号
        posMap.put("FIELD003",new DomainInfo(PosTypeEnum.INTEGER,"6"));//processing_code 交易处理码
        posMap.put("FIELD004",new DomainInfo(PosTypeEnum.INTEGER,"12"));//amt_trans 交易金额
        posMap.put("FIELD005",new DomainInfo(PosTypeEnum.INTEGER,"12"));//amt_settlmt 清算金额
        posMap.put("FIELD006",new DomainInfo(PosTypeEnum.INTEGER,"12"));//amt_cdhldr_bil 持卡人扣账金额
        posMap.put("FIELD007",new DomainInfo(PosTypeEnum.INTEGER,"10"));//transmsn_date_time 交易传输时间 （MMDDhhmmss）
        posMap.put("FIELD009",new DomainInfo(PosTypeEnum.INTEGER,"8"));//conv_rate_settlmt 清算汇率
        posMap.put("FIELD010",new DomainInfo(PosTypeEnum.INTEGER,"8"));//conv_rate_cdhldr_bil  持卡人扣账汇率
        ...
        posMap.put("FIELD016",new DomainInfo(PosTypeEnum.INTEGER,"4"));//date_conv 兑换日期 （MMDD）
        ...
        posMap.put("FIELD023",new DomainInfo(PosTypeEnum.INTEGER,"3"));//card_seq_id 卡序列号
        ...
        posMap.put("FIELD028",new DomainInfo(PosTypeEnum.STRING,"9"));//amt_trans_fee 交易费   1位符号位＋8位定长数字字符
        ...
        posMap.put("FIELD038",new DomainInfo(PosTypeEnum.STRING,"6"));//authr_id_resp 授权标识应答码
        ...
        posMap.put("FIELD044",new DomainInfo(PosTypeEnum.STRING,"VAR2"));//addtnl_resp_code 附加响应数据
        ...
        posMap.put("FIELD050",new DomainInfo(PosTypeEnum.STRING,"3"));//currcy_code_settlmt 清算货币代码
        posMap.put("FIELD051",new DomainInfo(PosTypeEnum.STRING,"3"));//currcy_code_cdhldr_bil 持卡人帐户货币代码
        posMap.put("FIELD052",new DomainInfo(PosTypeEnum.BINARY,"8"));//pin_data 个人标识码数据  b64
        ...
        posMap.put("FIELD055",new DomainInfo(PosTypeEnum.STRING,"VAR3"));//ICC_data IC卡数据域
        posMap.put("FIELD056",new DomainInfo(PosTypeEnum.STRING,"VAR2"));//par 标记化支付账户参考号（PAR）            !!!!VAR3
        posMap.put("FIELD057",new DomainInfo(PosTypeEnum.STRING,"VAR3"));//issr_addtnl_data 附加交易信息
        posMap.put("FIELD059",new DomainInfo(PosTypeEnum.STRING,"VAR3"));//detail_inqrng 明细查询数据
        ...
        posMap.put("FIELD070",new DomainInfo(PosTypeEnum.INTEGER,"3"));//netwk_mgmt_info_code 网络管理信息码
        posMap.put("FIELD090",new DomainInfo(PosTypeEnum.STRING,"42"));//orig_data_elemts 原始数据元
        posMap.put("FIELD096",new DomainInfo(PosTypeEnum.BINARY,"8"));//msg_security_code 报文安全码  b64
        posMap.put("FIELD100",new DomainInfo(PosTypeEnum.STRING,"VAR2"));//rcvg_inst_id_code 接收机构标识码
        posMap.put("FIELD102",new DomainInfo(PosTypeEnum.STRING,"VAR2"));//acct_id1 帐户标识1
        ...
        posMap.put("FIELD125",new DomainInfo(PosTypeEnum.STRING,"VAR3"));//risk_asse_info 安全和风险评估信息
        posMap.put("FIELD128",new DomainInfo(PosTypeEnum.BINARY,"8"));//msg_authn_code 报文鉴别码 b64
    }
